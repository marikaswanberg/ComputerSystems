...
struct Cache::Impl {
	...
	hash_func hasher_;
	index_type memused_;
	std::unordered_map<std::string, std::tuple<val_type, index_type>> unorderedmap_;
	std::queue<std::string> eviction_queue_;
public:
	Impl(index_type maxmem, hash_func hasher) 
	: maxmem_(maxmem), hasher_(hasher), memused_(0), unorderedmap_(), eviction_queue_(){
	...
	int set(key_type key, val_type val, index_type size){
		...
		std::tuple<val_type, index_type> entry = std::make_tuple((void*)copy, size);
	...
	val_type get(key_type key, index_type& val_size) const {
		std::tuple<val_type, index_type> entry;
		try{
	...
	void evictor() {
		key_type oldest_key = eviction_queue_.front();
		eviction_queue_.pop();
